...
void FileCompressHuff::CompressFile(const string& path)
{
	...
	FILE* fIn = fopen(path.c_str(),"rb");//这里说一下我为啥把文件以读写方式打开
	//因为假如用户要压缩的数据是同一种字符，那么建树的时候，就会出现一个问题那就是，只有一个结点，
	//建立哈夫曼树的时候，就是有一个节点，也就是没有权值，这样的话，在获取编码的时候，就没有办法获取权值，因为只有一个结点
	if (nullptr == fIn)
	{
	...
	//下面14行代码，主要是遍历字符种类，假如只有一种的话，就给哈夫曼树中多写一个' '，这样是为了建树，从而获取动态编码
	int zifuzhonglei = 0;
	...
	FILE* fOut = fopen("3.txt", "wb");  //要来保存压缩后的信息
	if (!fOut)
	{
		assert(false);
		return;
	...
	WriteHead(fOut, path);   //调用写入头信息函数；头部信息中主要包含，源文件后缀，以及头部信息的长度，以及每种字符出现的次数


	unsigned char ch = 0;  //ch用来进行位操作，来存压缩后的字符。A:"100",B"101",C"11",D:"0",ABC假如存在一个位中。那么ch的8个比特位分别就是11010111
	...
	//最后一次ch中可能不够8个比特位
	if (bitcount > 0 && bitcount < 8)  //这个错误害得我差点嗝屁
	{
...
void FileCompressHuff::WriteHead(FILE* fOut, const string& fileName) //最后一个参数就是文件的后缀
{
	...
	string strChCount;  //记录拼装的字符
	string strHead;  //记录源文件后缀
...
void FileCompressHuff::UNCompressFile(const string& path)  //解压缩主函数
{
	...
	//拿着压缩数据解压缩
	unsigned char buffer[1024] = { 0 };  //读取的缓冲区
	unsigned char ch = 0;  //每次往出读一个字符八个比特位
